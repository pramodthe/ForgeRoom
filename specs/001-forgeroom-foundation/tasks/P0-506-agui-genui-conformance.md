...
- `apps/e2e/helpers/trace-redaction.test.ts` proves the artifact scanner detects a secret canary that is absent from raw ZIP bytes but present after decompression, fails closed for unreadable archives, does not scrub a known fixture credential before evaluation, and verifies a real browser interaction-token round trip is absent when safe trace capture options are used; the same scanner covers JSON/text logs and is enforced by the prototype, live API and provider Playwright suites. Focused result: 6/6 passed.
