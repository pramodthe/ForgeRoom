...
- [ ] Repository layout matches `plan.md` or an accepted ADR.
- [ ] Shared packages include explicit AG-UI and controlled-component boundaries without loading the P1 generated-UI runtime or selecting unverified package versions ahead of P0-210.
- [ ] API and worker modules are separable even if one demo process starts both.
- [ ] Format, lint, typecheck, unit-test and production-build scripts exist.
- [ ] CI runs the same frozen-lockfile commands.
- [ ] Environment example lists names only and no secret is committed.
...
- CI run: `.github/workflows/ci.yml` added with frozen-lockfile lint/typecheck/test/build. Not executed on GitHub because this change is unpushed.
- Commands/results:
...
- 2026-08-25 — Review remediation completed: formatting fixed, the Node/Vite minimum aligned at 22.12.0, and canonical specifications included in the public repository. The full local CI command chain and API health probe passed on Node 22.12.0; hosted CI remains pending push.
...
- Open risks: GitHub Actions has not run on a pushed commit; Playwright is deferred to P0-504.
