...
- Official Hono/SSE client proof: `apps/api/src/ag-ui/routes.integration.test.ts` drives the real route through `HttpAgent`, a session cookie, expected Origin and current CSRF token; forged Origin/CSRF requests return typed `csrf_failed` before dispatch.
...
- Interactive continuation/reconnect proof: `packages/db/src/ui-interactions.integration.test.ts`, `packages/db/src/agent-turn-create-context.integration.test.ts`, `apps/worker/src/index.integration.test.ts`, and `apps/web/src/ag-ui/channel-timeline-reducer.test.ts` cover durable interrupt resolution, exactly one component continuation, replay and multi-wire-run logical-turn state.
